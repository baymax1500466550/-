...
DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_MODE_CHOOSE, DP_TYPE_ENUM},
  {DPID_COLOR_CHANGE, DP_TYPE_ENUM},
};
...
extern int switch_led_int;
    //此代码为平台自动生成，请按照实际数据修改每个可下发可上报函数和只上报函数
    mcu_dp_bool_update(DPID_SWITCH_LED,switch_led_int); //BOOL型数据上报;
//    mcu_dp_enum_update(DPID_MODE_CHOOSE,当前模式选择); //枚举型数据上报;
//    mcu_dp_enum_update(DPID_COLOR_CHANGE,当前自定义颜色切换); //枚举型数据上报;
...
/*****************************************************************************
  函数名称 : dp_download_mode_choose_handle
  功能描述 : 针对DPID_MODE_CHOOSE的处理函数
  输入参数 : value:数据源数据
        : length:数据长度
  返回参数 : 成功返回:SUCCESS/失败返回:ERROR
  使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_mode_choose_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为ENUM
  unsigned char ret;
  unsigned char mode_choose;
      extern int work_mode_int;
    extern int datachange1;

  mode_choose = mcu_get_dp_download_enum(value, length);
  switch (mode_choose) {
    case 0:
    Serial.println("3");
            datachange1=1;
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_enum_update(DPID_MODE_CHOOSE, mode_choose);
  if (ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
  函数名称 : dp_download_color_change_handle
  功能描述 : 针对DPID_COLOR_CHANGE的处理函数
  输入参数 : value:数据源数据
        : length:数据长度
  返回参数 : 成功返回:SUCCESS/失败返回:ERROR
  使用说明 : 可下发可上报类型,需要在处理完数据后上报处理结果至app
*****************************************************************************/
static unsigned char dp_download_color_change_handle(const unsigned char value[], unsigned short length)
{
  //示例:当前DP类型为ENUM
  unsigned char ret;
  unsigned char color_change;
      extern int color_change_int;
    extern int datachange2;
    extern int author;

  color_change = mcu_get_dp_download_enum(value, length);
  switch (color_change) {
    case 0:
    Serial.println("6");
    author = 1;
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_enum_update(DPID_COLOR_CHANGE, color_change);
  if (ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
  ...
  ***********************************/
  ...
  switch (dpid) {
    case DPID_SWITCH_LED:
      ...
      break;
    case DPID_MODE_CHOOSE:
      //模式选择处理函数
      ret = dp_download_mode_choose_handle(value, length);
      break;
    case DPID_COLOR_CHANGE:
      //自定义颜色切换处理函数
      ret = dp_download_color_change_handle(value, length);
      break;
...
extern int weather_now;
extern int datachange3;
  ...
  if (my_strcmp(name, "temp") == 0) {
    ...
    datachange3 = 1;
    Serial.write(value_string);
    String AAA = String(value_string);
    String string_to_hex = "";
    for (int i = 0; i < AAA.length(); i++) {
      string_to_hex += String(data[i], HEX);
    }
    if (string_to_hex == "e998b5e99ba8" || string_to_hex == "e5b08fe99ba8" || string_to_hex == "e99ba8") {
      Serial.println("rain");
      weather_now = 1;
    } else if (string_to_hex == "e699b4") {
      Serial.println("sun");
      weather_now = 2;
    }else{
